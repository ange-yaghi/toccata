...
def test_1():
    ...
    Utilities.jitter(input_buffer, 30)
    ...
    for start_clip in range(input_buffer.get_note_count()):
        decision_tree.seed_match(start_clip, input_buffer, library, timestamp)
        decision_tree.check_decisions(start_clip, input_buffer, timestamp)
        decision_tree.prune()

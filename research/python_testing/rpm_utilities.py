...
class Utilities(object):
    ...
    def create_random_bar_quantized(note_count, bar_length, grid_spaces, notes=256, name=""):
        ...
        for _ in range(note_count):
            note = MidiNote(random.randint(0, notes), random.randint(0, grid_spaces - 1) * grid_size, 0.0)
            new_segment.add_note(note)
    ...
    @staticmethod
    def reset_segment_shift(segment):
        if segment.get_note_count() == 0:
            return

        dx = -segment.notes[0].start
        for note in segment.notes:
            note.start += dx

    @staticmethod
    def concatenate_segments(segment0: MidiSegment, segment1: MidiSegment):
        new_segment = segment0.create_copy()
        offset = new_segment.length

        for note in segment1.notes:
            new_note = MidiNote(note.pitch, note.start + offset, note.length)
            new_segment.add_note(new_note)

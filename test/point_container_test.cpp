...
TEST(PointContainerTest, AddTest) {
	toccata::MusicPointContainer container;
	container.AddPoint({ 0 });
	container.AddPoint({ 1 });
...
TEST(PointContainerTest, RemoveTest) {
	toccata::MusicPointContainer container;
	container.AddPoint({ 0 });
	container.AddPoint({ 2 });
	container.AddPoint({ 1 });

	EXPECT_EQ(container.GetPoints()[0].Timestamp, 0);
	EXPECT_EQ(container.GetPoints()[1].Timestamp, 1);
	EXPECT_EQ(container.GetPoints()[2].Timestamp, 2);

	container.RemovePoint(1);
	EXPECT_EQ(container.GetCount(), 2);

	EXPECT_EQ(container.GetPoints()[0].Timestamp, 0);
	EXPECT_EQ(container.GetPoints()[1].Timestamp, 2);

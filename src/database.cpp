...
const char *toccata::Database::ReportColumns_v1_0 =
...
void toccata::Database::OpenDatabase() {
...
bool toccata::Database::DoesTableExist(const char *tableName) {
...
void toccata::Database::RenameTable(const char *name, const char *newName) {
    sqlite3_stmt *statement;
    char buffer[256];
    int result;
    bool found = true;
...
int toccata::Database::ReadSettingInt(const char *settingName) {
    int result;
    char buffer[1024];
    sqlite3_stmt *statement;

    sprintf_s(buffer, "SELECT * FROM %s WHERE Parameter = '%s'", "toccata_application_main", settingName);
...
void toccata::Database::ReadSettingString(const char *settingName, char *target) {
    int result;
    char buffer[1024];
    sqlite3_stmt *statement;
    ...
    strcpy_s(target, 1024, (const char *)value);

    sqlite3_finalize(statement);
}

void toccata::Database::FindPerformance(int performanceID, char *targetName) {
    int result;
    char buffer[1024];
    sqlite3_stmt *statement;

    sprintf_s(buffer, "SELECT * FROM %s WHERE PerformanceID = %d", "toccata_application_directory", performanceID);

    result = sqlite3_prepare(m_database, buffer, -1, &statement, NULL);
    result = sqlite3_step(statement);

    const unsigned char *value = sqlite3_column_text(statement, 1);
    strcpy_s(targetName, 1024, (const char *)value);
...
void toccata::Database::ReadPerformanceReport(int performanceID, PerformanceReport *report) {
    char tableName[1024];
    ...
    sprintf_s(buffer, "SELECT * FROM %s WHERE PerformanceID = %d", tableName, performanceID);
...
void toccata::Database::AddPerformance(int performanceID, const char *songName) {
    char buffer[256];
    sprintf_s(buffer, "%d, '%s'", performanceID, songName);

    Insert("toccata_application_directory", buffer);
}

void toccata::Database::CreateTable(const char *tableName, const char *columns) {
    char buffer[1024];
...
void toccata::Database::Insert(const char *tableName, const char *data) {
    char buffer[1024];
...
void toccata::Database::SimpleUpdate(const char *table, const char *idColumn, const char *id, const char *values) {
    char buffer[1024];
        ...
        result != SQLITE_DONE) 
    {
        ...
        toccata::Core::Get()->GetLogger()->Log(Logger::LogLevel::Debug, "SQL error while updating value.");
    }
}

void toccata::Database::InitializeDatabase() {
    ...
    if (!hasMainTable) {
        ...
        char buffer[256];
        sprintf_s(buffer, "'Version', %d", CurrentVersion);
        ...
        Settings.FileVersion = (DatabaseVersion)ReadSettingInt("Version");
...
void toccata::Database::PortDatabase(DatabaseVersion newVersion) {
    // To make life easier, porting only works in steps of 1
    while ((int)m_version != (int)newVersion - 1) {
        PortDatabase((DatabaseVersion)((int)m_version + 1));
    }

    if (newVersion == DatabaseVersion::v_1_0 /* Replace with VERSION_1_1 */ && m_version == DatabaseVersion::v_1_0) {
        // Do stuff
    }
}

void toccata::Database::WritePerformanceReport(PerformanceReport *performanceReport, const char *detailedFile, const char *midiFile) {
    /* Columns
    "Performance ID,"
    "Date,"
    "Tempo,"
    ...
    */
    ...
    sprintf_s(buffer, "toccata_report_%s", songName);
...
int toccata::Database::NewPerformanceID() {
    int newPerformanceID = Settings.PerformanceID++;

    char buffer[256];
    sprintf_s(buffer, "Value = %d", Settings.PerformanceID);
...
void toccata::Database::CreateTableName(char *name) {
    int i = 0;
    while (true) {
        if (name[i] == '\0') return;
        else if (name[i] == ' ') {
